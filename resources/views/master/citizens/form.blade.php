    ...
    <form method="POST" action="{{ $contextCard ? route($routePrefix.'.family-cards.members.store', $contextCard) : ($citizen->exists ? route($routePrefix.'.citizens.update', $citizen) : route($routePrefix.'.citizens.store')) }}" class="card border-0 shadow-sm">
        <div class="card-body p-4">@csrf @if($citizen->exists)@method('PUT')@endif
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6"><label for="marital_status" class="form-label">Status Perkawinan</label><select id="marital_status" name="marital_status" class="form-select"><option value="">Belum diisi</option>@foreach(\App\Enums\MaritalStatus::cases() as $status)<option value="{{ $status->value }}" @selected(old('marital_status', $citizen->marital_status?->value) === $status->value)>{{ $status->label() }}</option>@endforeach</select></div>
