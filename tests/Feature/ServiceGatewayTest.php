...
class ServiceGatewayTest extends TestCase
{
    ...
    public function test_whatsapp_action_uses_a_human_readable_territory_reference_without_a_technical_token(): void
    {
        ...
        $this->assertStringContainsString('MULAI LAPORAN SIGAP WARGA', $location);
        $this->assertStringContainsString("Pintu layanan:\n003 / 001", $location);
        $this->assertStringNotContainsString('[SW:', $location);
        $this->assertStringNotContainsString('swh_', $location);
    ...
    public function test_public_number_is_required(): void
    {
        ...
        $this->post($route, ['privacy_acknowledged' => '1'])->assertStatus(503);
        $this->assertDatabaseCount('service_handoffs', 0);
    }

    public function test_repeated_whatsapp_clicks_do_not_create_server_side_records(): void
    {
        $entry = app(ServiceEntryPointIssuer::class)->issue($this->createRt());
        $route = route('service-gateway.whatsapp', ['entryToken' => $entry->token]);

        foreach (range(1, 5) as $_) {
            $this->post($route, ['privacy_acknowledged' => '1'])->assertRedirect();
        }
    ...
    public function test_privacy_acknowledgement_is_required_before_opening_whatsapp(): void
    {
        ...
        $this->post(route('service-gateway.whatsapp', ['entryToken' => $entry->token]))
